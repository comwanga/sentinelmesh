...
mod tests {
    ...
    fn pending_advances_to_unverified_at_score_3() {
        assert_eq!(compute_new_status("PENDING", 3, 2, 0, 0, false), Some("UNVERIFIED".into()));
    }
    #[test]
    fn unverified_advances_to_verified_at_score_7() {
        assert_eq!(compute_new_status("UNVERIFIED", 7, 5, 0, 0, false), Some("VERIFIED".into()));
    }
    #[test]
    fn verified_advances_to_authoritative_at_score_15() {
        assert_eq!(compute_new_status("VERIFIED", 15, 10, 0, 0, false), Some("AUTHORITATIVE".into()));
    }
    #[test]
    fn rejected_when_score_minus_5_in_pending() {
        assert_eq!(compute_new_status("PENDING", -5, 0, 3, 0, false), Some("REJECTED".into()));
    }
    #[test]
    fn rejected_when_score_minus_5_in_disputed() {
        assert_eq!(compute_new_status("DISPUTED", -5, 0, 4, 0, false), Some("REJECTED".into()));
    }
    #[test]
    fn disputed_when_denial_count_dominates() {
        assert_eq!(compute_new_status("VERIFIED", 3, 1, 4, 0, false), Some("DISPUTED".into()));
    }
    #[test]
    ...
    fn rejection_takes_priority_over_positive() {
        assert_eq!(compute_new_status("PENDING", -5, 5, 0, 0, false), Some("REJECTED".into()));
    }
    ...
    fn gate_allows_verified_with_two_established_voters() {
        assert_eq!(compute_new_status("UNVERIFIED", 7, 5, 0, 2, true), Some("VERIFIED".into()));
    }
    #[test]
    ...
    fn gate_allows_authoritative_with_three_established() {
        assert_eq!(compute_new_status("VERIFIED", 15, 10, 0, 3, true), Some("AUTHORITATIVE".into()));
    }
}
