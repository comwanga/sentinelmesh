...
pub async fn run(pool: PgPool, private_key: String, subject: String) {
    ...
    loop {
        ...
        if let Err(error) = process_batch(&pool, &private_key, &subject, worker_id).await {
            tracing::warn!(%error, "push outbox cycle failed");
        }
    }
}
...
async fn process_batch(
    ...
    let deliveries: Vec<Delivery> = sqlx::query_as(
        "WITH claim AS (
         ...
         UPDATE push_deliveries d SET status = 'processing', worker_id = $2, locked_at = now(), updated_at = now()
    ...
    ).bind(BATCH_SIZE).bind(worker_id).fetch_all(pool).await?;
            ...
            match send(private_key, subject, &delivery).await {
                Ok(()) => {
                    if let Err(error) = mark_sent(pool, delivery.id, worker_id).await {
                        tracing::warn!(%error, "push completion write failed");
                    }
                }
                Err(error) => {
                    if let Err(db_error) = mark_failed(pool, delivery.id, worker_id, &error).await {
                        tracing::warn!(%db_error, "push retry write failed");
                    }
                }
            }
        })
...
async fn mark_sent(pool: &PgPool, id: Uuid, worker_id: Uuid) -> Result<()> {
    sqlx::query("UPDATE push_deliveries SET status='sent', sent_at=now(), locked_at=NULL, worker_id=NULL, updated_at=now() WHERE id=$1 AND worker_id=$2")
        .bind(id).bind(worker_id).execute(pool).await?;
    Ok(())
}
...
async fn mark_failed(
    ...
    ).bind(id).bind(worker_id).bind(MAX_ATTEMPTS).bind(error.to_string()).execute(pool).await?;
    Ok(())
}
