...
import reportReducer, { reportReceived, reportsHydrated } from '../store/reportSlice'
import type { CommunityReport } from '../../../../shared/types'

function makeReport(id: string): CommunityReport {
  return {
    id, report_type: 'FLOODING', description: null,
...
describe('reportSlice', () => {
  ...
  test('reportReceived prepends new report', () => {
    const state = reportReducer(undefined, reportReceived(makeReport('r1')))
    expect(state.items).toHaveLength(1)
    expect(state.items[0]!.id).toBe('r1')
  })

  test('reportReceived updates existing report in-place', () => {
    const initial = reportReducer(undefined, reportReceived(makeReport('r1')))
  ...
  test('caps items at 100', () => {
    const initial = reportReducer(undefined, reportsHydrated(Array.from({ length: 100 }, (_, i) => makeReport(`r${i}`))))
