...
test "$(compose exec -T postgres psql -U postgres -d sentinelmesh -Atc \
  "SELECT count(*) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'circle_members' AND column_name IN ('membership_state','accepted_at','key_wrap_epoch','key_wrap_event')")" = 4
test "$(compose exec -T postgres psql -U postgres -d sentinelmesh -Atc \
  "SELECT count(*) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'circles' AND column_name = 'membership_revision'")" = 1
test "$(compose exec -T postgres psql -U postgres -d sentinelmesh -Atc \
  "SELECT count(*) FROM circle_members WHERE circle_id = '00000000-0000-0000-0000-000000000043' AND member_token = 'v1:member' AND membership_state = 'ACTIVE' AND key_wrap_epoch IS NULL AND key_wrap_event IS NULL AND accepted_at IS NULL")" = 1
test "$(compose exec -T postgres psql -U postgres -d sentinelmesh -Atc \
