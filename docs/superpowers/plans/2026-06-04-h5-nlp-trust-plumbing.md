...
- **Known follow-ups for 2B:**
  - `origin_channel` and `h3_r9` are `NOT NULL` in `nlp_signals` but optional/derived in the payload; Phase 2B's staging insert must default a missing channel (e.g. to `source_type`) and compute `h3_r9`/`h3_r7` from lat/lng (always present) before insert — otherwise the insert fails.
  - **Radio provenance is currently `job_id` (per-job UUID), which is unique per capture** — two captures of the same broadcast look like independent sources and would over-count corroboration. 2B must key radio `source_id` on the stable station/stream id (e.g. `stream_id`) instead, or radio cannot self-deduplicate. Until then radio effectively always counts as distinct sources.
  - `source_type` and `origin_channel` are identical for all three current ingest sites. The split exists so a future channel can carry another source_type (e.g. a webhook delivering RSS content); 2B logic should branch on `origin_channel` for independence and treat `source_type` as descriptive only. Document this before 2B branches on either.
  - Twitter `source_id` depends on the stream requesting `author_id` (fixed in this phase by adding `expansions=author_id&user.fields=username` to the stream URL). 2B independence for Twitter remains author-level — a documented temporary heuristic per the spec.
